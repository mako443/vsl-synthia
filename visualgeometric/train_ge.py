...
from dataloading.data_loading import SynthiaDatasetTriplet
...
print(f'Geometric Embedding training: image limit: {IMAGE_LIMIT} ds: {DATASET} bs: {BATCH_SIZE} lr gamma: {LR_GAMMA} embed-dim: {EMBED_DIM_GEOMETRIC} shuffle: {SHUFFLE} margin: {MARGIN} lr:{LR}')
...
print('\n----')           
model_name=f'model_GeometricEmbed_l{IMAGE_LIMIT}_d{DATASET}_b{BATCH_SIZE}_g{LR_GAMMA:0.2f}_e{EMBED_DIM_GEOMETRIC}_s{SHUFFLE}_m{MARGIN}_lr{LR}.pth'
